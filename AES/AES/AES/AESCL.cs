...
namespace AES
{
    class AESCL
    {
        ...
        private byte[] key;
        private byte[,] Sbox, iSbox, w, Rcon, State;

        public AESCL(KeySize keySize, byte[] keyBytes)
        {
                // Устанавливаем Nb, Nk, Nr
            this.Nb = 4;
            if (keySize == KeySize.Bits128)
            {
                this.Nk = 4;  // 4 слова = 16 байт = 128 бит
                this.Nr = 10; // 10 раундов
            }
            if (keySize == KeySize.Bits192)
            {
                this.Nk = 6;  // 6 слов = 24 байта = 192 бит
                this.Nr = 12; // 12 раундов
            }
            if (keySize == KeySize.Bits256)
            {
                this.Nk = 8;  // 8 слов = 32 байта = 256 бит
                this.Nr = 14; // 14 раундов
            }
